...
const DEFAULT_MODE$1 = MODE_FIT$1;

/** The default resize timeout */
const DELAYED_RESIZE_MILLIS$1 = 200;
...
/**
 * @typedef {CustomEvent} ResizeEvent
 * @property {number} detail.width
 * @property {number} detail.height
 */

/**
 * @typedef {MODE_CENTER|MODE_FIT|MODE_NOSCALE|MODE_SCALE|MODE_FILL|MODE_STRETCH} FlexCanvasScaling
 */
 ...
 * For Canvas2D:
 * ```
 * const display = document.querySelector('flex-canvas');
...
class FlexCanvas extends HTMLElement {
   ...
   * @param {FlexCanvasScaling} [opts.mode]
  ...
  static define(customElements = window.customElements, opts = undefined) {
    const { tagName = 'flex-canvas' } = opts || {};
   ...
   * @returns {FlexCanvasScaling} The current scaling mode.
   */
  get mode() {
    return /** @type {FlexCanvasScaling} */ (this.getAttribute('mode'));
  }
  ...
  connectedCallback() {
    ...
    if (!this.hasAttribute('mode')) {
      this.setAttribute('mode', DEFAULT_MODE$1);
    }
    ...
    const innerSlot = this.shadowRoot.querySelector('#inner');
    innerSlot.addEventListener('slotchange', this.onSlotChange);
  ...
  attributeChangedCallback(attribute, prev, value) {
    ...
    switch (attribute) {
      ...
      case 'id':
      case 'class':
        this._titleElement.innerHTML = `display-port${this.className ? '.' + this.className : ''
   ...
   */
  onSlotChange(e) {
    let el = /** @type {HTMLSlotElement} */ (e.target);
    let nodes = el.assignedElements({ flatten: true });
    let canvas = /** @type {HTMLCanvasElement} */ (nodes.find(el => el instanceof HTMLCanvasElement));
    if (!canvas) {
  ...
  updateCanvasSize(force = true) {
    ...
    const mode = this.mode;
    if (mode === MODE_STRETCH$1 || mode === MODE_FILL$1) {
      canvasWidth = clientWidth;
      canvasHeight = clientHeight;
    } else if (mode !== MODE_NOSCALE$1) {
      if (
    ...
    if (force) {
      if (mode === MODE_SCALE$1) {
        canvas.width = this._width;
        canvas.height = this._height;
      } else if (mode !== MODE_STRETCH$1) {
      ...
      if (mode === MODE_FIT$1 || mode === MODE_FILL$1) {
...
export { DisplayPort, FlexCanvas, MODE_CENTER, MODE_FILL$1 as MODE_FILL, MODE_FIT$1 as MODE_FIT, MODE_NOSCALE$1 as MODE_NOSCALE, MODE_SCALE$1 as MODE_SCALE, MODE_STRETCH$1 as MODE_STRETCH };
