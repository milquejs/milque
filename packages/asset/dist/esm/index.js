async function TextLoader(url, opts = undefined) {
...
async function JSONLoader(url, opts = undefined) {
  let blob = await fetch(url);
  return blob.json();
}

async function OBJLoader(url, opts = undefined) {
  let blob = await fetch(url);
...
export { ImageLoader, JSONLoader, OBJLoader, TextLoader };
