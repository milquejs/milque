...
class InputMapElement extends HTMLElement {
  ...
  /** @override */
  static get observedAttributes() {
    return ['src', // Listening for built-in attribs
    'id', 'class'];
  }
...
class InputSourceElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    if (!this.hasAttribute('for')) this._setSourceElement(this); // Start animation frame loop
  ...
  disconnectedCallback() {
    ...
    cancelAnimationFrame(this._animationFrameHandle); // Terminate input source
  ...
  onAnimationFrame(now) {
    this._animationFrameHandle = requestAnimationFrame(this.onAnimationFrame); // If auto is enabled, do auto-polling

    if (this._autopoll) this.poll(); // If debug is enabled, do poll-counting
...
class InputContextElement extends HTMLElement {
  ...
  static get observedAttributes() {
    return ["for", "disabled", "autopoll", "debug", 'src', // Listening for built-in attribs
    'id', 'class'];
  }
...
export { AdapterManager, Axis, Button, Input, InputContext, InputContextElement, InputDevice, InputEventCode, InputKeyElement, InputMapElement, InputSource, InputSourceElement, InputSourceStage, InputType, KEY_STRING_DEVICE_SEPARATOR, Keyboard, Mouse, Synthetic, WILDCARD_DEVICE_MATCHER, WILDCARD_KEY_MATCHER, isInputAxis, parseKeyString, stringifyDeviceKeyCodePair };
