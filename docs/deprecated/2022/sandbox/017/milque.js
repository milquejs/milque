var self = /*#__PURE__*/ Object.freeze({
  ...
  get FineDiffStrategy() {
    return FineDiffStrategy;
  },
  get HotEntityModule() {
    return HotEntityModule;
  },
  get HotEntityReplacement() {
    return HotEntityReplacement;
  },
  get QueryOperator() {
    return QueryOperator;
  },
  get ReflexiveEntity() {
    return ReflexiveEntity;
  },
  get TagComponent() {
    return TagComponent;
  },
  ...
  get Random() {
    return DEFAULT_RANDOM_INTERFACE;
  },
  get RandomGenerator() {
...
class DisplayPort extends HTMLElement {
  ...
  update(now) {
    ...
    if (this.debug) {
      // Update FPS...
      const frames =
        delta <= 0 ? '--' : String(Math.round(1000 / delta)).padStart(2, '0');
  ...
  clear(fill = undefined) {
    if (fill) {
      ...
      this._canvasContext.fillRect(
        ...
        this._canvasElement.clientHeight
      );
    } else {
  ...
  static computeKey(components) {
    ...
    for (let component of components) {
      if (typeof component === 'object' && OPERATOR$1 in component) {
        result.push(
          component[OPERATOR$1].toString() + getComponentTypeName$1(component)
        );
      } else {
  ...
  select(entityManager) {
    ...
    if (this.persistent && !flag) {
      if (prevEntityManager) {
        ...
        prevEntityManager.componentHandler.off(
          'remove',
...
class EntityHandler {
  ...
  addEntityListener(entityId, eventType, listener, opts = undefined) {
    const handle =
      opts && typeof opts.handle !== 'undefined' ? opts.handle : listener;
...
class ComponentHandler {
  ...
  createComponent(componentType, initialValues) {
    ...
    if (type === 'object') {
      ...
      if (!('create' in componentType)) {
        throw new Error(
          `Instanced component class '${getComponentTypeName(
            componentType
          )}' must at least have a static create() function.`
        );
      }
  ...
  putComponent(
    ...
    if (componentInstanceMap.has(entityId)) {
      throw new Error(
        `Cannot add more than one instance of component class '${getComponentTypeName(
          componentType
        )}' for entity '${entityId}'.`
      );
    }
    ...
    this._entityHandler.dispatchEntityEvent(entityId, 'componentadd', [
      entityId,
      componentType,
      component,
      initialValues,
  ...
  deleteComponent(entityId, componentType, component) {
    ...
    this._entityHandler.dispatchEntityEvent(entityId, 'componentremove', [
      entityId,
      componentType,
      component,
    ]);
...
class EntityManager {
  ...
  addComponent(entityId, componentType, initialValues = undefined) {
    try {
      let component = this.componentHandler.createComponent(
        componentType,
        initialValues
      );
      this.componentHandler.putComponent(
        entityId,
        componentType,
        component,
        initialValues
      ...
      console.error(
        `Failed to add component '${getComponentTypeName$1(
          componentType
        )}' to entity '${entityId}'.`
      );
      console.error(e);
  ...
  addTagComponent(entityId, componentType) {
    try {
      ...
      if (type === 'symbol') {
        ...
        throw new Error(
          `Component of type '${type}' cannot be a tag component.`
        );
      }
      return componentType;
    } catch (e) {
      console.error(
        `Failed to add tag component '${getComponentTypeName$1(
          componentType
        )}' to entity '${entityId}'.`
      );
      console.error(e);
  ...
  removeComponent(entityId, componentType) {
    try {
      ...
      console.error(
        `Failed to remove component '${getComponentTypeName$1(
          componentType
  ...
  clearComponents(entityId) {
    for (let componentInstanceMap of this.componentHandler.getComponentInstanceMaps()) {
      if (componentInstanceMap.has(entityId)) {
        ...
        this.componentHandler.deleteComponent(
          entityId,
          componentType,
          component
        );
  ...
  getComponentTypesByEntityId(entityId) {
    ...
    for (let componentType of this.componentHandler.getComponentTypes()) {
      let componentInstanceMap =
        this.componentHandler.getComponentInstanceMapByType(componentType);
      if (componentInstanceMap.has(entityId)) {
...
const Not = createQueryOperator(function NotOperator(
  world,
  entityId,
  componentTypees
) {
...
function defaultReset(dst) {
  Object.getOwnPropertyNames(dst).forEach((value) => delete dst[value]);
}
...
class ReflexiveEntity extends EntityBase {
  constructor(entityManager) {
    ...
    this.entityManager.entityHandler.addEntityListener(
      this.id,
      'componentadd',
      this.onComponentAdd
    );
  ...
  onComponentRemove(entityId, componentType, component) {
    ...
    if (componentType === EntityComponent$1) {
      this.entityManager.entityHandler.removeEntityListener(
        this.id,
        'componentadd',
        this.onComponentAdd
      );
  ...
  if (typeof component === 'object') {
    ...
    for (let prop of Object.getOwnPropertyNames(component)) {
      if (ownProps.includes(prop)) {
        throw new Error(
          `Conflicting property names in entity for component '${getComponentTypeName(
            componentType
          )}'.`
        );
      }
...
function computeDiff(source, target, path = [], opts = {}) {
  ...
  for (let key of Object.getOwnPropertyNames(target)) {
    if (!sourceKeys.has(key)) {
      ...
      let result = computeDiff$4(
        source[key],
        target[key],
        nextProperty(path, key),
        opts
      );
      if (!result) {
...
const DEFAULT_HANDLERS = [ArrayObjectDiff, MapDiff, SetDiff];
...
function FineDiffStrategy(
  ...
  for (let componentType of cacheEntityManager.getComponentTypesByEntityId(
    newEntity
  )) {
    let newComponent = cacheEntityManager.getComponent(
      newEntity,
      componentType
    );
    let oldComponent = cacheEntityManager.getComponent(
      oldEntity,
      componentType
    );
  ...
  for (let entityManager of prevEntityManagers) {
    // Update entities...
    for (let entity of prevHotEntityModule.entities
      .get(entityManager)
      .values()) {
...
class HotEntityModule {
  ...
  replaceWith(nextHotEntityModule, opts = undefined) {
    ...
    for (let entityManager of nextHotEntityModule.entityManagers) {
      for (let entity of nextHotEntityModule.entities
        .get(entityManager)
        .values()) {
...
function enableForEntity(entityModule, entityManager, entityId) {
  if (!HOT_ENTITY_MODULES.has(entityModule.id)) {
    throw new Error(
      'Module must be accepted first for HER to enable hot entity replacement.'
    );
  }
...
function acceptForModule(
  entityModule,
  entityConstructor,
  worldConstructor = undefined
) {
  ...
  return {
    ...
    setPriority(priority) {
      if (priority > MAX_CONTEXT_PRIORITY || priority < MIN_CONTEXT_PRIORITY) {
        throw new Error(
          `Context priority must be between [${MIN_CONTEXT_PRIORITY}, ${MAX_CONTEXT_PRIORITY}].`
        );
      }
...
class ApplicationLoop extends HTMLElement {
  ...
  attributeChangedCallback(attribute, prev, value) {
    switch (attribute) {
      // Event handlers...
      case 'onstart':
        this._onstart = new Function(
          'event',
          `with(document){with(this){${value}}}`
        ).bind(this);
        break;
      case 'onstop':
        this._onstop = new Function(
          'event',
          `with(document){with(this){${value}}}`
        ).bind(this);
        break;
      case 'onpreupdate':
        this._onpreupdate = new Function(
          'event',
          `with(document){with(this){${value}}}`
        ).bind(this);
        break;
      case 'onupdate':
        ...
        break;
      case 'onfixedupdate':
        this._onfixedupdate = new Function(
          'event',
          `with(document){with(this){${value}}}`
        ).bind(this);
        break;
      case 'onpostupdate':
        this._onpostupdate = new Function(
          'event',
          `with(document){with(this){${value}}}`
        ).bind(this);
        break;
      case 'onpause':
        this._onpause = new Function(
          'event',
          `with(document){with(this){${value}}}`
        ).bind(this);
        break;
      case 'onresume':
        this._onresume = new Function(
          'event',
          `with(document){with(this){${value}}}`
        ).bind(this);
        break;
  ...
  onAnimationFrame(now) {
    if (this._controlled)
      throw new Error('Cannot run controlled game loop; call step() instead.');
  ...
  step(now = ApplicationLoop.currentTime()) {
    ...
    if (this.prevAccumulatedTime > MAX_FIXED_UPDATES * this.fixedTimeStep) {
      ...
      console.error(
        `[ApplicationLoop] Too many updates! Skipped ${count} fixed updates.`
      );
    }
  ...
  set onpreupdate(value) {
    if (this._onpreupdate)
      this.removeEventListener('preupdate', this._onpreupdate);
  ...
  get onfixedupdate() {
    return this._onfixedupdate;
  }
  set onfixedupdate(value) {
    if (this._onfixedupdate)
      this.removeEventListener('fixedupdate', this._onfixedupdate);
  ...
  set onpostupdate(value) {
    if (this._onpostupdate)
      this.removeEventListener('postupdate', this._onpostupdate);
...
async function load(game) {
  ...
  let instance =
    (game.load && (await game.load(game))) ||
...
function start(instance) {
  ...
  let viewport = instance.viewport || {
    ...
    y: 0,
    get width() {
      return displayPort.getCanvas().clientWidth;
    },
    get height() {
      return displayPort.getCanvas().clientHeight;
    },
  };
  ...
  applicationLoop.addEventListener('update', gameInfo.onfirstupdate, {
    once: true,
  });
...
function applyMotion(
  entity,
  inverseFrictionX = 1,
  inverseFrictionY = inverseFrictionX
) {
  ...
  update(dt) {
    if (this._transition) {
      ...
      if (currentScene) {
        ...
        if ('unload' in currentScene)
          result = result.then(() => currentScene.unload(this.sharedContext));
      }

      if ('load' in nextScene)
        result = result.then(() =>
...
class Transform2D {
  ...
  get rotation() {
    return Math.atan2(-this.matrix[2], this.matrix[0]);
  }
  set rotation(value) {
...
class Camera2D extends AbstractCamera {
  ...
  getProjectionMatrix() {
    ...
    return [
      this.transform.matrix[0],
      0,
      0,
      this.transform.matrix[3],
      this.offsetX,
      this.offsetY,
    ];
  }
...
function drawWorldGrid(ctx, view, camera) {
  ...
  const maxCellSize = Math.floor(
    (Math.log(view.width) - Math.log(minCellWidth)) * INV_NATURAL_LOG_2
  );
...
class DefaultRandomInterface extends RandomInterface {
  constructor() {
    super(new RandomGenerator());
  }

  withGenerator(generator) {
    return new RandomInterface(generator);
  }
  withSeed(seed) {
    return new RandomInterface(new SimpleRandomGenerator(seed));
  }
}
...
export {
  ...
  PriorityQueue,
  QueryOperator,
  DEFAULT_RANDOM_INTERFACE as Random,
  RandomGenerator,
  RandomInterface,
  RangeInputAdapter,
  ReflexiveEntity,
