export class PhysicsSystem {
  ...
  update(dt) {
    ...
    let collisions = aabbGraph.solve(
      entityManager.getComponentEntityIds('Motion')
    );
    for (let collision of collisions) {
      ...
      }
      {
        ...
        if (
          entityManager.has('Motion', entityId) &&
          entityManager.has('Transform', entityId)
        ) {
          if (entityManager.has('Solid', otherId)) {
            let solid = entityManager.get('Solid', otherId);
            if (
              !solid.masks ||
              solid.masks.length <= 0 ||
              solid.masks.includes(collision.otherMask.name)
            ) {
              ...
              if (
                collision.hit.nx &&
                Math.sign(collision.hit.nx) === Math.sign(motion.motionX)
              ) {
                motion.motionX = 0;
              }
              if (
                collision.hit.ny &&
                Math.sign(collision.hit.ny) === Math.sign(motion.motionY)
              ) {
                motion.motionY = 0;
              }
            }
