...
export function resolve(componentType) {
  switch (typeof componentType) {
    case 'string':
      return createComponentFactoryFromOptions(
        componentType,
        true,
        undefined,
        undefined,
    ...
    default:
      throw new Error(
        `Unsupported component factory option type for ${componentType}`
      );
  }
}
...
function createComponentFactoryFromOptions(
  componentType,
  tag,
  template,
  create,
  ...
  if (tag) {
    if (template) {
      throw new Error(
        `Invalid component factory options for ${componentType} - ` +
