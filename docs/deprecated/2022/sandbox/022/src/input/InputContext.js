...
export class InputContext {
  ...
  _setupInputs(inputSource, inputMap) {
    ...
    const isPrevSourceReplaced =
      prevInputSource !== inputSource && prevInputSource;
  ...
  toggle(force = this._disabled) {
    if (force) {
      if (!this.inputSource) {
        throw new Error(
          'Input source must be set before enabling input context.'
        );
      }

      if (Object.keys(this.inputs).length <= 0) {
        console.warn(
          'No inputs found for enabled input context - did you forget to setInputMap()?'
        );
      }
    }
