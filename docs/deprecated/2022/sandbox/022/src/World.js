...
export class World {
  static getWorld(opts = {}) {
    ...
    if (!(WORLD_STATIC_INSTANCE in this)) {
      if (!lazy) {
        throw new Error(
          'Invalid immediate access to uninitialized world. Must call World.provide() before this or be lazily loaded.'
        );
      }
  ...
  static provide(values) {
    ...
    if (required.size <= 0) {
      ...
      throw new Error(
        `Missing required world dependencies: ${Array.from(required)}`
      );
    }
  }
