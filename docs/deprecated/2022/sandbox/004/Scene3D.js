...
export function init(gl) {
  const context = {};
  context.shaderProgram = Mogli.createShaderProgramInfo(
    gl,
    DEFAULT_VERTEX_SHADER_SOURCE,
    DEFAULT_FRAGMENT_SHADER_SOURCE,
    DEFAULT_SHARED_ATTRIBUTE_LAYOUT
  );
  context.camera = createPerspectiveCamera();

  context.sharedUniforms = {
    get u_projection() {
      return context.camera.projectionMatrix;
    },
    get u_view() {
      return context.camera.viewMatrix;
    },
    get u_camera() {
      return context.camera.transformMatrix;
    },
  };
  return context;
}
...
export function createPerspectiveCamera(fovy = 1, near = 0.01, far = 1000) {
  return {
    ...
    update(gl, displayWidth, displayHeight) {
      ...
      Mogli.Transform.getTransformationMatrix(
        this.transform,
...
export function createMesh(gl, geometry, sharedAttributeLayout) {
  ...
  const positionBuffer = Mogli.createBufferInfo(
    gl,
    gl.FLOAT,
    geometry.position,
    3
  );
  const texcoordBuffer = Mogli.createBufferInfo(
    gl,
    gl.FLOAT,
    geometry.texcoord,
    2
  ...
  const indicesBuffer = Mogli.createElementBufferInfo(
    gl,
    gl.UNSIGNED_SHORT,
    geometry.indices
  );
...
export function createModelGeometry(
  ...
  return createModel(
    program,
    createMesh(gl, geometry, DEFAULT_SHARED_ATTRIBUTE_LAYOUT, color)
  );
}
