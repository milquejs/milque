...
export function onUpdate(dt) {
  ...
  if (this.player.activeTradeTarget) {
    ...
    if (Utils.distance2D(this.player, tradePost) > tradePost.tradeRadius) {
      ...
      // Keep in orbit...
      this.player.dx = Utils.lerp(
        this.player.dx,
        dx * ORBIT_VELOCITY,
        dt * ORBIT_SPEED
      );
      this.player.dy = Utils.lerp(
        this.player.dy,
        dy * ORBIT_VELOCITY,
        dt * ORBIT_SPEED
      );
      ...
      if (actionControl) {
        ...
        let tradeCooldown =
          (MAX_PLAYER_TRADE_COOLDOWN - MIN_PLAYER_TRADE_COOLDOWN) *
            (1 - Utils.clampRange(this.player.activeTradeFactor / 5, 0, 1)) +
          MIN_PLAYER_TRADE_COOLDOWN;
  ...
  this.player.dx = Utils.clampRange(
    this.player.dx,
    -MAX_PLAYER_VELOCITY,
    MAX_PLAYER_VELOCITY
  );
  this.player.dy = Utils.clampRange(
    this.player.dy,
    -MAX_PLAYER_VELOCITY,
    MAX_PLAYER_VELOCITY
  );
  ...
  if (this.camera.target) {
    this.camera.x = Utils.lerp(
      this.camera.x,
      this.camera.target.x,
      dt * this.camera.speed
    );
    this.camera.y = Utils.lerp(
      this.camera.y,
  ...
  for (let trainCart of this.trainCarts) {
    if (
      trainCart.parent &&
      Utils.distance2D(trainCart, trainCart.parent) > TRAIN_CART_RADIUS * 2
    ) {
      trainCart.prevX = trainCart.x;
      trainCart.prevY = trainCart.y;
      trainCart.x = Utils.lerp(
        trainCart.x,
        trainCart.parent.x,
        dt * TRAIN_CART_SPEED
      );
      trainCart.y = Utils.lerp(
        trainCart.y,
        trainCart.parent.y,
        dt * TRAIN_CART_SPEED
      );
...
export function onRender(view, world) {
  ...
  ctx.translate(cameraX, cameraY);
  {
    for (const tradePost of this.tradePosts) {
      Utils.drawCircle(
        ...
        tradePost.x,
        tradePost.y,
        tradePost.tradeRadius - PLAYER_RADIUS,
        'lime',
        true
      );
      Utils.drawBox(
      ...
      if (tradePost.tradeMode) {
        Utils.drawText(
          ctx,
          (Math.sign(tradePost.trade[0]) > 0 ? '+' : '-') +
            '$' +
            Math.abs(tradePost.trade[0]) +
            ' => ' +
            tradePost.trade[1] +
            ' ' +
            tradePost.trade[2],
    ...
    for (const trainCart of this.trainCarts) {
      let rotation = Math.atan2(
        trainCart.y - trainCart.prevY,
        trainCart.x - trainCart.prevX
      );
      Utils.drawBox(
        ctx,
        trainCart.x,
  ...
  for (let itemName of Object.keys(this.player.inventory)) {
    ...
    ctx.fillText(
      `${this.player.inventory[itemName]} x ${itemName}`,
      8,
      view.height - 8 - 16 * itemLine++
    );
  }
