...
export function onUpdate(dt) {
  ...
  this.player.rotation = Utils.lookAt2D(
    this.player.rotation,
    this.player.heading,
    dt * PLAYER_ROTATION_DELTA
  );
  this.player.x +=
    Math.cos(this.player.heading) * this.player.speed + this.player.dx;
  this.player.y +=
    Math.sin(this.player.heading) * this.player.speed + this.player.dy;
  ...
  for (let gravityWell of gravityWells) {
    ...
    if (gravityWell.orbitable) {
      ...
      if (distance < orbitDistance + GRAVITY_WELL_ORBIT_PADDING) {
        ...
        let orbitDistanceRatio =
          (distance - orbitDistance) / GRAVITY_WELL_ORBIT_PADDING;
        ...
        targetHeading =
          direction -
          Utils.clampRange((orbitDistanceRatio + 1) * Math.PI, 0, Math.PI);

        isDoingOrbit = orbitDistanceRatio < -1 || !player.moving;
        if (isDoingOrbit) {
          player.heading = Utils.lookAt2D(
            player.heading,
            targetHeading,
            dt * GRAVITY_WELL_ORBIT_ROTATION_DELTA
          );
          if (
            player.orbitting &&
            Utils.lookAt2D(player.heading, targetHeading, 1) !== player.heading
          ) {
            player.speed = Utils.lerp(
              player.speed,
              0,
              dt * GRAVITY_WELL_ORBIT_DAMPING_FORCE
            );
          } else {
            player.speed = Utils.lerp(
              player.speed,
              PLAYER_CONTROL_MIN_SPEED,
              dt * GRAVITY_WELL_ORBIT_FORCE
            );
          }
        }
      }
...
function applyGravity(source, target, radius) {
  ...
  if (distance < radius) {
    ...
    let ratio = Math.min(
      MAX_GRAVITY_WELL_PULL_FACTOR,
      1 - distance / (radius * GRAVITY_WELL_PULL_MAGNITUDE)
    );
...
function createExhaustParticles(
  ...
  for (let i = amount; i >= 0; --i) {
    ...
    game.entities.addComponent(entityId, ExhaustParticle, {
      x: x + Random.randomRange(-radius, radius),
      y: y + Random.randomRange(-radius, radius),
