  ...
  let result = {
    ...
    nextScene(scene, transition = undefined, context = undefined) {
      if (!this._nextScene) {
        ...
        this._nextContext =
          context || (Object.isExtensible(scene) ? scene : {});
        ...
        throw new Error(
          'Cannot change scenes while in the middle of a scene transition'
        );
      }
    },
    start() {
    ...
    update(dt) {
      if (this._transition) {
        ...
        if ('load' in nextScene)
          result = result.then(() => nextScene.load.call(nextContext, this));
    ...
    _renderStep(
      ...
      if (renderer)
        renderer.call(renderContext, view.context, view, this.world);
