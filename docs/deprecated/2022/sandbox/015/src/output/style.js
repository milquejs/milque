...
function fuel(amount, max = amount, change = 0) {
  if (amount <= 0) {
    ...
    return `[${chalk.magenta('|'.repeat(amount + change))}${chalk.gray(
      '|'.repeat(-change)
    )}${chalk.gray('*'.repeat(max - amount))}]`;
  } else {
    return `[${chalk.magenta('|'.repeat(amount))}${chalk.magenta(
      '*'.repeat(change)
    )}${chalk.gray('*'.repeat(max - amount - change))}]`;
  }
}
