...
export function drawWorldGrid(ctx, viewWidth, viewHeight, view = IDENTITY_MATRIX, projection = IDENTITY_MATRIX)
{
    ...
    const maxCellSize = Math.floor((Math.log(viewWidth) - Math.log(minCellWidth)) * INV_NATURAL_LOG_2);
    
    let cellWidth = Math.pow(2, maxCellSize) * minCellWidth;
    let cellHeight = Math.pow(2, maxCellSize) * minCellHeight;
    if (cellWidth === 0 || cellHeight === 0) return;
    drawGrid(ctx, viewWidth, viewHeight, offsetPoint.x, offsetPoint.y, cellWidth, cellHeight, 1, false);
    drawGrid(ctx, viewWidth, viewHeight, offsetPoint.x, offsetPoint.y, cellWidth / 2, cellHeight / 2, 3 / 4, false);
    drawGrid(ctx, viewWidth, viewHeight, offsetPoint.x, offsetPoint.y, cellWidth / 4, cellHeight / 4, 2 / 4, false);
    drawGrid(ctx, viewWidth, viewHeight, offsetPoint.x, offsetPoint.y, cellWidth / 8, cellHeight / 8, 1 / 4, false);
}

export function drawWorldTransformGizmo(ctx, viewWidth, viewHeight, view = IDENTITY_MATRIX, projection = IDENTITY_MATRIX)
{
    const viewMatrix = new DOMMatrixReadOnly(view);
...
export function drawGrid(ctx, viewWidth, viewHeight, offsetX, offsetY, cellWidth = 32, cellHeight = cellWidth, lineWidth = 1, showCoords = false)
{
    ctx.beginPath();
