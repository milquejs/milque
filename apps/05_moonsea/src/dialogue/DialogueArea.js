...
const [TYPEWRITE_KEYFRAMES, INVERSE_TYPEWRITE_KEYFRAMES] = (() => {
  let keyframes = [];
  let invKeyframes = [];
...
export class DialogueArea extends HTMLElement {
  ...
  step() {
    ...
    const outer = /** @type {HTMLElement} */ (
      this._typeWriteQueue[this._typeWriteIndex]
    );
    ...
    const wait =
      (inner.hasAttribute('data-p-wait')
