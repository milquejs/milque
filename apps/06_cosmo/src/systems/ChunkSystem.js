import { coords, createChunkDataById, createChunkMap, getChunkIdByCoords, getChunkOriginById, getTileByOffset, hasChunkDataById, setTile } from '../core/ChunkMap.js';
...
export function ChunkSystem(m) {
    ...
    addListener(m, 'render', (ctx) => {
        let chunkX = 0;
        let chunkY = 0;
        for (let [x, y] of coords(v, chunkX - 1, chunkY - 1, 0, chunkX + 1, chunkY + 1, 0)) {
            let chunkId = getChunkIdByCoords(map, x, y, 0);
            if (!hasChunkDataById(map, chunkId)) {
                continue;
            }
