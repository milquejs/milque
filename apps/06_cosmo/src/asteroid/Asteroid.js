...
import { useDraw, useUpdate } from './main.js';
...
export function AsteroidSystem(m) {
    ...
    useUpdate(m, ({ deltaTime: dt }) => {
        // Update asteroids
        updateAsteroids(dt, scene, scene.asteroids);
        // Update spawners
        updateAsteroidSpawner(dt, scene, scene.asteroidSpawner);
    });

    useDraw(m, ASTEROIDS_DRAW_LAYER_INDEX, (ctx) => {
        drawAsteroids(ctx, scene, scene.asteroids);
    });
}
 ...
 * @param {AsteroidGame} scene 
 */
function updateAsteroidSpawner(dt, scene, spawner) {
    if (scene.gamePause) {
        return;
    }
 ...
 */
function updateAsteroids(dt, scene, asteroids) {
 ...
 * @param {Array<ReturnType<spawnAsteroid>>} asteroids
 */
function drawAsteroids(ctx, scene, asteroids) {
    for (let asteroid of asteroids) {
