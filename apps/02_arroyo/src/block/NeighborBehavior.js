...
export function onBlockPlace(world, blockPos, blockId) {
  ...
  let neighbor = 0b0000;
  if (
    worldMap.isWithinBounds(blockPos.right(out)) &&
  ...
  if (
    worldMap.isWithinBounds(blockPos.up(out)) &&
    worldMap.getBlockId(out) === blockId
  ) {
    neighbor |= 0b0010;
  ...
  if (
    worldMap.isWithinBounds(blockPos.left(out)) &&
    worldMap.getBlockId(out) === blockId
  ) {
    neighbor |= 0b0100;
  ...
  if (
    worldMap.isWithinBounds(blockPos.down(out)) &&
    worldMap.getBlockId(out) === blockId
  ) {
    neighbor |= 0b1000;
