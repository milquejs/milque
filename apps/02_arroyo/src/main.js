...
function updatePlacement(game, dt) {
  ...
  function onPlace(placeState) {
    ...
    const [centerX, centerY] = Camera2D.screenToWorld(
      display.width / 2,
      display.height / 2,
      viewMatrix,
      projectionMatrix
...
function initializeWorld(world, display) {
  ...
  PlacementSystem.placeBlock(
    world,
    blockPos.offset(out, -1, 0),
    Blocks.GOLD.blockId
  );
  PlacementSystem.placeBlock(
    world,
    blockPos.offset(out, 0, -1),
    Blocks.GOLD.blockId
  );
  PlacementSystem.placeBlock(
    world,
    blockPos.offset(out, -1, -1),
    Blocks.GOLD.blockId
  );
