...
export function generateBoxyIsland(island, width, height, opts)
{
    ...
    for(let y = 1; y < height - 1; ++y)
    {
        for(let x = 1; x < width - 1; ++x)
        {
            ...
            if (ground[i])
            {
                let factor = ground[(x + 1) + (y + 1) * width]
...
export function digLine(out, width, height, fromX, fromY, toX, toY, radius = 1)
{
