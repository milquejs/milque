export class Loop {
  constructor(
    ...
    maxFixedStepsPerFrame = fixedStepsPerSecond * 1.5
  ) {
    this._prevTime = 0;
  ...
  onAnimationFrame(now) {
    ...
    if (
      ...
      this._maxAccumulatedTime + this._fixedTimeStep
    ) {
      let skippedTime = this._accumulatedTime - this._maxAccumulatedTime;
      console.warn(
        `Too many updates! Skipping ${Math.floor(
          skippedTime / this._fixedTimeStep
        )} update(s)...`
      );
      this._accumulatedTime = 0;
    }
