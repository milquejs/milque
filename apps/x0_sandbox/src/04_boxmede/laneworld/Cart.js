...
export class Cart
{
    constructor(cartManager, junctionMap, id, juncX, juncY)
    {
        this.id = id;
 ...
 * @param {JunctionCoord} juncX 
 * @param {JunctionCoord} juncY 
 */
export function createCart(cartManager, junctionMap, juncX, juncY)
{
    let cartId = uuid();
    let cart = new Cart(cartManager, junctionMap, cartId, juncX, juncY);
    cartManager.carts[cartId] = cart;
    let juncIndex = getJunctionIndexFromCoords(junctionMap, juncX, juncY);
    if (tryAcquireParkingInJunction(cartManager, junctionMap, cartId, juncIndex))
    {
        forceCartOnJunction(cartManager, junctionMap, cartId, juncIndex);
    }
    else
    {
 ...
 * @param {LaneWorld} world 
 */
export function updateTraffic(cartManager, junctionMap)
{
    ...
    for(let cart of Object.values(cartManager.carts))
    {
        if (cart.lastUpdatedTicks < worldTicks)
        {
            ...
            if (isNullJunction(junctionMap, cart.currentOutlet))
            {
                targetOutlet = updateNavigation(cartManager, junctionMap, cart.id);
