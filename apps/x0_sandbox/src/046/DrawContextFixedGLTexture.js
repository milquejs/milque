...
import { DrawContextFixedGLShape, QUAD_VERTICES } from './DrawContextFixedGLShape';
...
export class DrawContextFixedGLTexture extends DrawContextFixedGLShape {
    ...
    resize() {
        super.resize();
        this.texturedProgram.bind(this.gl).uniform('u_projection_view', this.projectionViewMatrix);
    }
    ...
    setTextureImage(textureUnit, image) {
        ...
        if (textureUnit < 0 || textureUnit >= maxTextureCount) {
            throw new Error(`Invalid texture unit - must be within range [0, ${maxTextureCount - 1}]`);
        }
        const gl = this.gl;
