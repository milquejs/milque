...
BouncingBall.onUpdate = function onUpdate(dt, world) {
  for (let ball of world.balls) {
    ...
    if (ball.x > WORLD_VIEW.width) {
      ball.x = WORLD_VIEW.width;
      xFlag = true;
    }
    ...
    if (ball.y > WORLD_VIEW.height) {
      ball.y = WORLD_VIEW.height;
      yFlag = true;
    }
...
function splitBall(world, ball) {
  ...
  const speed = Math.max(
    MAX_BALL_SPEED,
    Random.randomRange(prevSpeed, prevSpeed * 2)
  );
  ...
  BouncingBall(
    ...
    ball.x,
    ball.y,
    ball.width * BALL_SPLIT_FACTOR,
    ball.height * BALL_SPLIT_FACTOR,
    dx,
    dy
  );
  BouncingBall(
