export class TileMap {
  ...
  performCarve() {
    ...
    if (
      tileX >= 0 &&
      tileY >= 0 &&
      tileY < this.array.length &&
      tileX < this.array[tileY].length
    ) {
  ...
  updatePointer(view, x, y) {
    this.pointer.x =
      Math.trunc((x * view.width) / this.tileSize) * this.tileSize;
    this.pointer.y =
      Math.trunc((y * view.height) / this.tileSize) * this.tileSize;
  }
  ...
  draw(ctx) {
    ...
    drawTilePointer(
      ...
      this.pointer.x,
      this.pointer.y,
      this.registry[this.pointer.value],
      this.tileSize
    );
  }
}
