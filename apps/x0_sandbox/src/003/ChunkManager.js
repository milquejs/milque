...
export class ChunkManager {
  ...
  async performChunkLoading(world, chunkLoader, chunkId, chunk) {
    ...
    try {
      ...
      throw new Error(
        `Unable to load chunk ${chunkId} at chunk coords (${chunk.chunkX},${chunk.chunkY}) - ${e.message}`
      );
    }
  }
  ...
  unloadIdleChunks(world) {
    ...
    for (let chunk of this._chunks.values()) {
      ...
      if (this._loadingChunkIds.has(chunkId)) continue;
      if (
        currentChunkTime - this._lastActiveTimes.get(chunkId) >=
        chunk.maxChunkIdleTime
      ) {
        this.unloadChunk(world, chunkId);
      }
    }
  ...
  markActive(world, chunkId) {
    this._lastActiveTimes.set(
      chunkId,
      this.constructor.getCurrentChunkTime(world)
    );
  }
  ...
  isChunkIdle(chunkId) {
    if (this._lastActiveTimes.has(chunkId)) {
      let idleTime =
        this.constructor.getCurrentChunkTime(world) -
        this._lastActiveTimes.get(chunkId);
      let chunk = this._chunks.get(chunkId);
