...
export async function main(game) {
  ...
  for (let key in dungeonAtlas) {
    ...
    for (let i = 0; i < frames; ++i) {
      ...
      let x = u + w * i;
      spriteRenderer.frame(frameName, 1, x, v, x + w, v + h);
      ...
      spriteRenderer.pushTranslation(-camera.screenX, -camera.screenY);
      {
        ...
        for (let i = -1; i <= 1; ++i) {
          for (let j = -1; j <= 1; ++j) {
            ...
            if (chunk) {
              renderChunk(spriteRenderer, chunk, tileRenderer, {
                isTargetChunk: i === 0 && j === 0,
              });
            }
          }
        }
        ...
        let [playerScreenX, playerScreenY] = isoToCart(
          vec2.create(),
          player.x,
          player.y
        );
        spriteRenderer.draw(
          'dungeon.lizard_m_idle_anim',
          Math.floor(frameTime * 0.5),
          playerScreenX - halfCubeWidth,
          playerScreenY - halfCubeHeight
        );
        ...
        spriteRenderer.draw(
          'cube',
          0,
          cursor.screenX - halfCubeWidth,
          cursor.screenY - halfCubeHeight
        );
      }
