 ...
 * @typedef {(src: string|Uint8Array) => Promise<T>} AssetLoader
 */
            ...
            const result = await Promise.race([
                ...
                new Promise((resolve, reject) => setTimeout(() => {
                    if (this._loadResolves.length > 0) {
                        reject(new Error('Timeout reached while waiting for asset ref binding.'));
                    } else {
...
async function tryLoadFromSource(assetPack, uri, source, loader, timeout) {
    ...
    if (loader) {
        if (typeof result !== 'string' && !(result instanceof Uint8Array)) {
            throw new Error('Unable to load from source - must be either a string or Uint8Array.');
        }
