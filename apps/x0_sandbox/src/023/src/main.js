...
async function main() {
  ...
  const mainProgram = GLUtil.createProgramInfo(
    gl,
    GLUtil.Program(gl)
  ...
  const mainCameraController = CameraUtil.createFirstPersonCameraController({
    locky: true,
  });
  ...
  const cactusParts = (() => {
    ...
    let plantRightArm = createGameObject(
      -0.9,
      -1 + armY,
      0,
      0.2,
    ...
    let plantRightUpArm = createGameObject(
      -1,
      -0.7 + armY,
      0,
      0.2,
    ...
    let plantLeftArm = createGameObject(
      0.9,
      -1.3 + armY,
      0,
      0.2,
    ...
    let plantLeftUpArm = createGameObject(
      1,
      -1 + armY,
      0,
      0.2,
  ...
  display.addEventListener('frame', (e) => {
    ...
    const moveX =
      input.getInputValue('MoveRight') - input.getInputValue('MoveLeft');
    const moveZ =
      input.getInputValue('MoveUp') - input.getInputValue('MoveDown');
    ...
    const lookSpeed = 100;
    mainCameraController.look(
      lookX * lookSpeed * aspectRatio,
      -lookY * lookSpeed
    );
    ...
    let ray = CameraUtil.screenToWorldRay(
      2 * (eyeX - 0.5),
      2 * (0.5 - eyeY),
