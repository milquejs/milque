...
export class EntityManager {
   ...
   */
  constructor(opts = {}) {
    const { componentRegistry = undefined, strictMode = false } = opts;

    /** @type {Map<EntityId, import('./ComponentFactory.js').ComponentFactory>} */
    this.components =
      componentRegistry ||
      (strictMode ? new Map() : new AutoFactoryResolverMap());
  ...
  create(entityId = String(this.nextAvailableEntityId++)) {
    if (this.strictMode) {
      if (typeof entityId !== 'string')
        throw new Error(`Invalid non-string type for entity id '${entityId}'.`);
      if (this.entities.has(entityId))
        throw new Error(
          `Invalid duplicate entity id '${entityId}' allocated for new entity.`
        );
    }
  ...
  add(componentType, entityId, props = undefined) {
    if (this.strictMode) {
      if (!componentType) {
        throw new Error(
          `Cannot add null or undefined component type ${getComponentTypeName(
            componentType
          )}`
        );
      }
      ...
      if (typeof componentType !== 'string' && !('name' in componentType)) {
        throw new Error(
          `Unnamed component types are not supported - ${JSON.stringify(
            componentType
          )}`
        );
      }
      ...
      if (!this.components.has(componentType)) {
        if (typeof componentType === 'string') {
          throw new Error(
            `Found unregistered tag component ${getComponentTypeName(
              componentType
            )}.`
          );
        } else {
          throw new Error(
            `Missing component factory for ${getComponentTypeName(
              componentType
            )}.`
          );
        }
      ...
      if (typeof entityId !== 'string') {
        throw new Error(
          `Invalid entity id type; expected string but found ${typeof entityId} instead.`
        );
      }
