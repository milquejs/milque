...
(function () {
  ...
  function screenToWorldRay(out, normalizedScreenCoordX, normalizedScreenCoordY, projectionMatrix, viewMatrix, normalized = false) {
    ...
    let v = fromValues$1(normalizedScreenCoordX, normalizedScreenCoordY, -1, 1); // To camera coords

    let m = create();
    invert(m, projectionMatrix);
    transformMat4(v, v, m);
    v[2] = -1;
  ...
  class OrthographicCamera extends Camera {
    ...
    resize(viewportWidth, viewportHeight) {
      ...
      ortho(this.projectionMatrix, left * aspectRatio, right * aspectRatio, bottom, top, near, far);
      return this;
    }
  ...
  class QuadRendererBase {
    ...
    constructor(gl) {
      ...
      this._vector = create$1();
    }

    setTransformationMatrix(matrix) {
      copy(this._transformMatrix, matrix);
      return this;
    }
    ...
    updateModelMatrix(posX, posY, scaleX, scaleY) {
      let m = this._matrix;
      let v = this._vector;
      copy(m, this._transformMatrix);
      translate(m, m, set(v, posX, posY, 0));
      scale(m, m, set(v, scaleX, scaleY, 1));
      return m;
    }
    /** @abstract */
...
void main()
{
  ...
  class TexturedQuadRenderer extends QuadRendererBase {
    ...
    constructor(gl) {
      super(gl);
      this._texture = null;
      this._textureSize = fromValues$2(1, 1);
      this._spriteVector = fromValues$1(0, 0, 1, 1);
      this._colorVector = fromValues(1, 1, 1);
    }

    setTexture(texture, w, h) {
      this._texture = texture;
      set$2(this._textureSize, w, h);
      return this;
    }

    setSprite(u = 0, v = 0, s = 1, t = 1) {
      set$1(this._spriteVector, u, v, s, t);
      return this;
    }

    setColor(r = 1, g = 1, b = 1) {
      set(this._colorVector, r, g, b);
      return this;
    }
    /** @override */
    ...
    draw(posX = 0, posY = 0, scaleX = 1, scaleY = 1) {
      ...
      programInfo.bind(gl).attribute('a_position', gl.FLOAT, positionBufferInfo.handle).attribute('a_texcoord', gl.FLOAT, texcoordBufferInfo.handle).uniform('u_projection', projection).uniform('u_view', view).uniform('u_model', model).uniform('u_texture', 0).uniform('u_sprite', sprite).uniform('u_color', color).uniform('u_texture_size', textureSize).draw(gl, gl.TRIANGLES, 0, 6);
    }
...
void main()
{
  ...
  class ColoredQuadRenderer extends QuadRendererBase {
    ...
    draw(posX = 0, posY = 0, scaleX = 1, scaleY = 1) {
      ...
      let model = this.updateModelMatrix(posX, posY, scaleX, scaleY);
      let color = this._colorVector;
      programInfo.bind(gl).attribute('a_position', gl.FLOAT, bufferInfo.handle).uniform('u_projection', projection).uniform('u_view', view).uniform('u_model', model).uniform('u_color', color).draw(gl, gl.TRIANGLES, 0, 6);
    }
  ...
  function GameLoader(game) {
    ...
    return async function () {
      ASSETS.registerAsset('texture', 'font', 'cloud/cube.png', {
        gl
      });
  ...
  function GameUpdater(game) {
    ...
    return function () {
      ...
      screenToWorldRay(create$1(), screenX, screenY, camera.projectionMatrix, camera.viewMatrix);
    };
  }
  ...
  function GameRenderer(game) {
    ...
    return function () {
      ...
      texturedRenderer.setProjectionMatrix(camera.projectionMatrix).setViewMatrix(camera.viewMatrix);
      coloredRenderer.setProjectionMatrix(camera.projectionMatrix).setViewMatrix(camera.viewMatrix);
