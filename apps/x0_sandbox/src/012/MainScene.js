...
export async function load(game) {
  ...
  game
    .addRenderTarget(HUD_VIEW, onHUDRender)
...
export function onHUDRender(ctx, view, world) {
  RenderHelper.drawNavigationInfo(
    view,
...
export function onWorldRender(ctx, view, world) {
  ...
  const mouseX =
    MouseControls.POS_X.value * view.width +
    world.camera.transform.x -
    world.camera.offsetX;
  const mouseY =
    MouseControls.POS_Y.value * view.height +
    world.camera.transform.y -
    world.camera.offsetY;
