...
export function drawCarts(
  ...
  for (let cart of Object.values(cartManager.carts)) {
    ...
    if (currentJunction !== -1 && currentOutlet !== -1) {
      ...
      let remainingPrevSlots =
        cart.prevSlot !== NULL_SLOT_INDEX ? LANE_SLOT_COUNT - cart.prevSlot : 0;
...
function getLanePosition(map, inletIndex, outletIndex, laneIndex, progress) {
  ...
  return [inx + 0.5 + lx + dx * progress, iny + 0.5 + ly + dy * progress];
}
