  ...
  moveAgents(agents) {
    ...
    for (let agent of agents) {
      ...
      if (!isNullJunction(map, targetIndex)) {
        ...
        if (speed > 0) {
          let remaining = moveAgentTowards(
            map,
            agent,
            targetIndex,
            agent.intent,
          ...
          if (speed < remaining) {
            throw new Error(
              'Agent gained speed after moving? How is that possible?'
            );
          }
        }
...
export function moveAgentTowards(map, agent, outletIndex, intent, speed) {
  ...
  if (!junc.hasOutlet(outletIndex)) {
    throw new Error(
      `Missing outlet '${getJunctionCoordsFromIndex(
        map,
        outletIndex
      )}' at junction '${getJunctionCoordsFromIndex(
        map,
        juncIndex
      )}' to move cart towards.`
    );
  }
  ...
  if (isNullJunction(map, agent.outlet)) {
    ...
    if (
      furthest >= lane.length &&
      nextSlot >= lane.length &&
      intent !== NO_JUNCTION_INTENT
    ) {
      switch (intent) {
        case PASSING_JUNCTION_INTENT:
          if (isNullJunction(map, agent.nextTarget)) {
            ...
            isJunctionPassable(map, outletIndex) &&
            canJunctionLaneAcceptMore(map, outletIndex, agent.nextTarget)
          ) {
...
function canJunctionLaneAcceptMore(junctionMap, inletIndex, outletIndex) {
  if (inletIndex < 0 || outletIndex < 0)
    throw new Error('Junction index must be non-negative.');
  let lane = getJunctionLaneByIndex(junctionMap, inletIndex, outletIndex);
  if (!lane)
    throw new Error(
      `Cannot find lane for given inlet '${getJunctionCoordsFromIndex(
        junctionMap,
        inletIndex
      )}' and outlet '${getJunctionCoordsFromIndex(junctionMap, outletIndex)}'.`
    );
