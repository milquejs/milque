...
export class JunctionMap {
  ...
  getJunction(juncIndex) {
    ...
    if (!result) {
      throw new Error(
        `Cannot get non-existant junction at index ${juncIndex}.`
      );
    }
    return result;
  }
  ...
  deleteJunction(juncIndex) {
    ...
    if (!prev) {
      throw new Error(
        `Cannot delete non-existant junction at index ${juncIndex}.`
      );
    }
...
export class Junction {
  ...
  removeInlet(juncIndex) {
    ...
    if (i >= 0) {
      ...
      throw new Error(
        `Cannot remove non-existant inlet ${juncIndex} for junction.`
      );
    }
  }
  ...
  removeOutlet(juncIndex) {
    ...
    if (i >= 0) {
      ...
      if (!prev) {
        throw new Error(
          `Cannot delete non-existant lane for outlet ${juncIndex}.`
        );
      }
      this.lanes[juncIndex] = undefined;
    } else {
      throw new Error(
        `Cannot remove non-existant outlet ${juncIndex} for junction.`
      );
    }
  }
  ...
  getLane(outletIndex) {
    ...
    if (!result) {
      throw new Error(
        `Cannot get non-existant lane for outlet ${outletIndex}.`
      );
    }
    return result;
  }
...
export function getJunctionCoordsFromIndex(map, juncIndex) {
  if (typeof map !== 'object') throw new Error('Missing map.');
  return [juncIndex % map.width, Math.floor(juncIndex / map.width)];
}
