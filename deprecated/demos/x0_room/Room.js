...
function MainScene()
{
    ...
    const models = [];
    const floor = Scene3D.createModelGeometry(gl, scene.shaderProgram, Mogli.Geometry.Plane.create(false));
    ...
    const wallLeft = Scene3D.createModelGeometry(gl, scene.shaderProgram, Mogli.Geometry.Plane.create(false));
    ...
    const wallRight = Scene3D.createModelGeometry(gl, scene.shaderProgram, Mogli.Geometry.Plane.create(false));
    ...
    const wallBack = Scene3D.createModelGeometry(gl, scene.shaderProgram, Mogli.Geometry.Plane.create(false));
    ...
    const background = Scene3D.createModelGeometry(gl, scene.shaderProgram, Mogli.Geometry.Plane.create(false));
    ...
    // RENDERING...
    Milque.Game.on('update', (dt) => {
        Scene3D.draw(gl, scene, models.map(model => model.drawInfo));
    });
}
