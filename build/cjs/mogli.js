...
function getTransformationMatrix(transform) {
  var dst = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : glMatrix.mat4.create();
...
function applyTransformation(transformationMatrix, geometry) {
  ...
  var inverseTransposeMatrix = glMatrix.mat3.create();
...
function create$3() {
  ...
  if (doubleSided) {
    ...
    var transformationMatrix = glMatrix.mat4.fromYRotation(glMatrix.mat4.create(), Math.PI);
...
function create$8() {
  var size = 1;
  var fifthSize = size * 0.2;
  var transformationMatrix = glMatrix.mat3.create();
