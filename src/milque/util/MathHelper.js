 ...
 * @returns {Object} dst
 */
export function getDirectionalVector(x1, y1, x2, y2, magnitude = 1, angleOffset = 0, dst = [0, 0])
{
    const dx = x2 - x1;
    const dy = y2 - y1;
 ...
 * @returns {Object} dst
 */
export function getMidPoint(x1, y1, x2, y2, dst=[0, 0])
{
    dst[0] = x1 + (x2 - x1) / 2;
    dst[1] = y1 + (y2 - y1) / 2;
    return dst;
...
export function distanceSqu(x1, y1, x2, y2)
{
    const dx = x2 - x1;
    const dy = y2 - y1;
    return dx * dx + dy * dy;
