...
export class Tia {
  constructor() {
    ...
    /** @private */
    this.projectionMatrix = mat4.fromTranslation(
      mat4.create(),
  ...
  spr(ctx, spriteImage, spriteIndex, x, y, w, h, flipX = false, flipY = false) {
    // @ts-ignore
    let imageW = Number(spriteImage.width);
    let dx = Math.floor(imageW / w);
  ...
  pop() {
    if (this.transformStack.length > 0) {
      let result = /** @type {mat4} */ (this.transformStack.pop());
